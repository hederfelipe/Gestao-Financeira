...
{
    public class Utilidade_ValidaCpf
    {
        ...
        public bool ValidaCPF(string cpf)
        {
            ...
            cpf = cpf.Trim(); cpf = cpf.Replace(".", "").Replace("-", "");
            ...
            if (resto < 2) resto = 0;
            else resto = 11 - resto; digito = resto.ToString();

            tempCpf = tempCpf + digito; soma = 0;
            for (int i = 0; i < 10; i++) soma += int.Parse(tempCpf[i].ToString()) * multiplicador2[i];
            resto = soma % 11; if (resto < 2) resto = 0;
